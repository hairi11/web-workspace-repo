...
test('ChoiceInput uses radio for fewer than five options', function () {
...
test('ChoiceInput uses select for five or more options', function () {
    assert.equal(ChoiceInput.resolveMode(5), ChoiceInput.Mode.SELECT);
...
test('ChoiceInput supports a custom threshold', function () {
    ...
    assert.equal(ChoiceInput.resolveMode(3, 3), ChoiceInput.Mode.SELECT);
});
